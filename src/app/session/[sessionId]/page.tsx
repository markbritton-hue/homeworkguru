          ...
          <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid var(--border)", background: "var(--surface)", backdropFilter: "blur(10px)" }}>
            ...
            >
              <div className="flex items-center gap-2 font-semibold text-xs">
                ...
                Homework sheet{session.imageDataUrls.length > 1 ? `s (${session.imageDataUrls.length} pages)` : ""}
              </div>
                ...
                {session.imageDataUrls.map((url, i) => (
                  <div key={i}>
                    {session.imageDataUrls.length > 1 && (
                      <p className="text-xs font-semibold px-4 py-1.5" style={{ color: "var(--muted)", background: "var(--input-bg)", borderBottom: "1px solid var(--border)" }}>
                        Page {i + 1}
                      </p>
                    )}
                    ...
                    <img src={url} alt={`Homework page ${i + 1}`}
                      ...
                      style={{ background: "var(--input-bg)", borderBottom: i < session.imageDataUrls.length - 1 ? "1px solid var(--border)" : undefined }} />
                  </div>
                ))}
              </div>
            )}
